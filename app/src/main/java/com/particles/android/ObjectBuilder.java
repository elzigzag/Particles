...
import com.particles.android.Geometry.Square;
...
public class ObjectBuilder {
    ...
    static GeneratedData createSquare(float x, float y, float z, float t){
        int size = 6;
        Square cuadro_temp = new Square(new Point(x, y, z), t);
        ObjectBuilder builder = new ObjectBuilder(size);
        builder.appendSquare(cuadro_temp.scale(1f));
        return builder.build();
    }

    public void appendSquare(Square square_x){

        final int startVertex = offset / FLOATS_PER_VERTEX;
        final int numVertices = 6;
        //x 1
        vertexData[offset++] = square_x.center.x;
        //y
        vertexData[offset++] = square_x.center.y;
        //z
        vertexData[offset++] = square_x.center.z;

        //x 2
        vertexData[offset++] = square_x.center.x - square_x.radius;
        //y
        vertexData[offset++] = square_x.center.y - square_x.radius;
        //z
        vertexData[offset++] = square_x.center.z;
